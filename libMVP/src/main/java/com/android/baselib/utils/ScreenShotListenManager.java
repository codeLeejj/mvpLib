...
public class ScreenShotListenManager {
    ...
    private ScreenShotListenManager(Context context) {
        ...
        if (sScreenRealSize == null) {
            ...
            if (sScreenRealSize != null) {
                DebugLog.d(TAG, "Screen Real Size: " + sScreenRealSize.x + " * " + sScreenRealSize.y);
            } else {
                DebugLog.w(TAG, "Get screen real size failed.");
            }
        }
    }
    ...
    public void startListen() {
        ...
        DebugLog.w("ScreenShotListener", "开始注册", mStartListenTime);
        // 创建内容观察者
    ...
    private void handleMediaContentChange(Uri contentUri) {
        DebugLog.w("ScreenShotListener", "收到变动通知");
        Cursor cursor = null;
        try {
            ...
            if (cursor == null) {
                DebugLog.e(TAG, "Deviant logic.");
                return;
            }
            if (!cursor.moveToFirst()) {
                DebugLog.d(TAG, "Cursor no data.");
                return;
            }
    ...
    private void handleMediaRowData(String data, long dateTaken, int width, int height) {
        if (checkScreenShot(data, dateTaken, width, height)) {
            DebugLog.d(TAG, "ScreenShot: path = " + data + "; size = " + width + " * " + height
                    + "; date = " + dateTaken);
            ...
            DebugLog.w(TAG, "Media content changed, but not screenshot: path = " + data
    ...
    private boolean checkScreenShot(String data, long dateTaken, int width, int height) {
         ...
         * 判断依据一: 时间判断
         */
        // 如果加入数据库的时间在开始监听之前, 或者与 当前时间相差大于10秒, 则认为当前没有截屏
        long interval = System.currentTimeMillis() -dateTaken;
        if (dateTaken < mStartListenTime || interval > 10 * 1000) {
            return false;
        }
